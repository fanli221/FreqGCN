...
def compute_stats(diffusion, multimodal_dict, model, logger, cfg):
    ...
    stats_meter = {x: {y: AverageMeter() for y in ['FreqGCN']} for x in stats_names}
    ...
    for i in tqdm(range(0, K), position=0):
        ...
        if i == K - 1:  # in last iteration, concatenate all candidate pred
            ...
            for j in range(0, num_samples):
                apd, ade, fde, mmade, mmfde = compute_all_metrics(pred[:, j, :, :],gt_group[j][np.newaxis, ...],traj_gt_arr[j])
                stats_meter['APD']['FreqGCN'].update(apd)
                stats_meter['ADE']['FreqGCN'].update(ade)
                stats_meter['FDE']['FreqGCN'].update(fde)
                stats_meter['MMADE']['FreqGCN'].update(mmade)
                stats_meter['MMFDE']['FreqGCN'].update(mmfde)
            for stats in stats_names:
                str_stats = f'{stats}: ' + ' '.join(
                    [f'{x}: {y.avg:.3f}' for x, y in stats_meter[stats].items()]
                )
    ...
    with open(file_latest % cfg.result_dir, 'w') as csv_file:
        writer = csv.DictWriter(csv_file, fieldnames=['Metric'] + ['FreqGCN'])
        writer.writeheader()
        for stats, meter in stats_meter.items():
            new_meter = {x: y.avg for x, y in meter.items()}
            new_meter['FreqGCN'] = new_meter['FreqGCN'].cpu().numpy()
    ...
    if os.path.exists(file_stat % cfg.result_dir) is False:
        ...
        df = pd.concat([df2, df1['FreqGCN']], axis=1, ignore_index=True)
